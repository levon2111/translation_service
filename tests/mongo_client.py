...
class MongoHandler:
    def __init__(self, db_name: str, collection_name: str):
        ...
        self.__db_client = AsyncIOMotorClient(os.environ.get("MONGO_DB_TEST"))
    ...
    async def insert_word(self, word: dict):
        await self.__db_client[self.__db_name][self.__collection_name].insert_one(word)
    ...
    async def __create_mock_data(self):
        with open("tests/mock_data/word.json", "r") as f:
            ...
            for word in word_json:
                word["create_time"] = datetime.strptime(word["create_time"], "%Y-%m-%d %H:%M:%S")
                word["update_time"] = datetime.strptime(word["update_time"], "%Y-%m-%d %H:%M:%S")
