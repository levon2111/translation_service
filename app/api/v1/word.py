...
from app.schemas.schemas import get_word, create_word, delete_word, get_words

router = APIRouter()


@router.get('/', include_in_schema=False, status_code=200)
@router.get('', response_model=WordInDB, status_code=200,
            responses={
                400: {}
            })
async def get_word_details(word: str, target_language_code: str, db: AsyncIOMotorClient = Depends(get_db),
    ...
    if db_word:
        ...
        options = [Translation(language=target_language_code, translation=i.translated_text) for i in
                   translation.translations]
        word = await create_word(db, word=word, translations=options)

        data = jsonable_encoder(word)

        return Response(data, message=messages.SUCCESS,
...
async def word_list(skip: int = Query(0, ge=0), limit: int = Query(10, ge=1),
